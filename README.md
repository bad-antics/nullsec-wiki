<div align="center">

# 📚 NullSec Wiki

**Official Documentation for the NullSec Security Framework**

[![Tools](https://img.shields.io/badge/Tools-135+-red?style=for-the-badge)](https://github.com/bad-antics/nullsec-linux)
[![NullSec Linux](https://img.shields.io/badge/NullSec_Linux-v5.0-blue?style=for-the-badge)](https://github.com/bad-antics/nullsec-linux)
[![NullKia](https://img.shields.io/badge/NullKia-v3.0-green?style=for-the-badge)](https://github.com/bad-antics/nullkia)
[![Discord](https://img.shields.io/badge/Discord-killers-5865F2?style=for-the-badge&logo=discord&logoColor=white)](https://discord.gg/killers)

</div>

---

## 📖 Table of Contents

### Getting Started
- [Home](Home.md) — Overview and quick links
- [Installation](Installation.md) — Getting NullSec up and running
- [Quick Start Guide](Quick-Start.md) — First steps with NullSec
- [System Requirements](Requirements.md) — Hardware and software requirements

### NullSec Linux v5.0
- [NullSec Linux Overview](NullSec-Linux.md) — Distribution documentation
- [Editions Guide](Editions.md) — Standard, Cloud, AI/ML, Hardware, Automotive
- [Architecture Support](Architecture.md) — AMD64, ARM64, RISC-V, Apple Silicon
- [Tool Categories](Tool-Categories.md) — 135+ tools organized by domain
- [Building from Source](Building.md) — Compile your own ISO

### NullKia v3.0 Mobile Security
- [NullKia Overview](NullKia.md) — Mobile security framework
- [Supported Manufacturers](Manufacturers.md) — 18 supported OEMs
- [Baseband Tools](Baseband.md) — Shannon, Qualcomm, MediaTek exploitation
- [Cellular Security](Cellular.md) — 5G/LTE, eSIM, IMSI tools
- [TEE & TrustZone](TEE.md) — Secure Element and BootROM research
- [Plugin Development](Plugins.md) — Extending NullKia

### Tool Documentation
- [Cloud Security Tools](Cloud-Tools.md) — AWS, GCP, Azure, K8s, Terraform
- [AI/ML Security Tools](AI-ML-Tools.md) — LLM red team, prompt injection
- [Hardware Security Tools](Hardware-Tools.md) — SDR, RFID, JTAG, glitch
- [Automotive Security Tools](Automotive-Tools.md) — CAN, OBD-II, UDS
- [Network Security Tools](Network-Tools.md) — Scanning, analysis, exploitation
- [Cryptography Tools](Crypto-Tools.md) — Hash, encryption, signatures

### Development
- [Contributing](Contributing.md) — How to contribute to NullSec
- [Code Style Guide](Code-Style.md) — Coding standards and conventions
- [API Reference](API.md) — Tool APIs and integration
- [Building Tools](Building-Tools.md) — Creating new NullSec tools

### Community
- [FAQ](FAQ.md) — Frequently asked questions
- [Troubleshooting](Troubleshooting.md) — Common issues and solutions
- [Changelog](Changelog.md) — Version history and updates
- [Security Policy](Security.md) — Responsible disclosure

---

## 🚀 Quick Links

| Resource | Description |
|----------|-------------|
| [🐧 NullSec Linux](https://github.com/bad-antics/nullsec-linux) | Security distribution with 135+ tools |
| [📱 NullKia](https://github.com/bad-antics/nullkia) | Mobile security framework |
| [🌐 Portal](https://bad-antics.github.io) | Official website |
| [💬 Discord](https://discord.gg/killers) | Community support |
| [📦 All Repos](https://github.com/bad-antics) | 373 repositories |

---

## 📊 Framework Statistics

| Component | Count |
|-----------|-------|
| Security Tools | 135+ |
| Programming Languages | 15+ |
| Linux Editions | 9 |
| Architectures | 4 |
| Mobile Manufacturers | 18 |
| Public Repositories | 373 |

---

<div align="center">

**© 2024-2026 bad-antics** • [GitHub](https://github.com/bad-antics) • [Discord](https://discord.gg/killers)

</div>
